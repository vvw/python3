...
def find_matrix_inverse(A):
    ...
    Examples:
        ...
        True
    '''
    # number of vectors: as much as len(A.D[0])
    # domain of new vectors: A.D[1]
    # imv => identity matrix vectors
    imv = [Vec(A.D[1], {idx:one}) for idx in A.D[0]]
    # determine each B columb from A*b[i] = imv[i]
    b_cols = [solve(A, vec) for vec in imv]
    B = coldict2mat(b_cols)
    return B
...
def find_triangular_matrix_inverse(A):
    ...
    # domain of new vectors: A.D[1]
    # imv => identity matrix vectors
    imv = [Vec(A.D[1], {idx:1}) for idx in A.D[0]]
    # determine each B columb from A*b[i] = imv[i]
    b_cols = [solve(A, vec) for vec in imv]
    B = coldict2mat(b_cols)
    return B
